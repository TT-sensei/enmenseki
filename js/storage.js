...
export const freshData=()=>({version:STORAGE_VERSION,stats:{},mistakes:[],weakTags:{},lastRoute:'#home',settings:{...DEFAULT_SETTINGS},cards:[],currentStreak:0,bestStreak:0});
export function loadData(){try{const raw=localStorage.getItem(STORAGE_KEY);if(!raw)return freshData();const parsed=JSON.parse(raw);if(parsed.version!==STORAGE_VERSION)return migrate(parsed);return{...freshData(),...parsed,settings:{...DEFAULT_SETTINGS,...parsed.settings}}}catch(error){console.warn('進捗データを復旧しました',error);return freshData()}}
