...
function circleConceptSvg(count, step) {
  ...
  for (let index = 0; index < teeth; index++) {
    const x = 345 + index * width;
    const topPath = rearrangedSlicePath({ x, width, topY: 52, centerY: 125, bottomY: 198, top: true });
    const bottomPath = rearrangedSlicePath({ x, width, topY: 52, centerY: 125, bottomY: 198, top: false });
    top += `<path class="slice-piece" d="${topPath}" fill="${index % 2 ? '#f7b68b' : '#82bad5'}" stroke="#fff"/>`;
    bottom += `<path class="slice-piece" d="${bottomPath}" fill="${index % 2 ? '#82bad5' : '#f7b68b'}" stroke="#fff"/>`;
  }
  return `<svg viewBox="0 0 640 255" role="img" aria-label="円を${count}分割し、上下交互に並べ替える図">
    ...
    <g opacity="${step === 0 ? '.25' : '1'}"><rect x="345" y="52" width="250" height="146" rx="3" fill="none" stroke="#17324d" stroke-width="1.5" stroke-dasharray="5 5"/>${top}${bottom}<line x1="345" y1="125" x2="595" y2="125" stroke="#17324d" stroke-width="1" stroke-dasharray="3 5"/><line x1="345" y1="211" x2="595" y2="211" stroke="#17324d"/><text x="470" y="240" text-anchor="middle">横＝円周の半分</text><text x="325" y="130" transform="rotate(-90 325 130)" text-anchor="middle">縦＝半径</text></g>
  </svg>`;
}
